...
def extract_base(image_true=None, trained_model=None, image_size=384, BUFFER=10):
...
def extract(image_true=None, trained_model=None, image_size=384, BUFFER=10, scale_factor= 1.05):
    global preprocess_transforms
    ...
    corners = order_points(corners)
    # Tính toán các điểm đích mở rộng
    destination_corners = find_dest(corners)

    # Áp dụng phép biến đổi phối cảnh với các điểm góc mở rộng
    M = cv2.getPerspectiveTransform(np.float32(corners), np.float32(destination_corners))
